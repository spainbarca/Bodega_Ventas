    ...
    public partial class OpcionesProductook : Form
    {
        ...
        private void MostrarCat()
        {
            try
            {
                ...
                datalistadoCat.Columns[2].Visible = false;
        ...
        private void btnSaveCat_Click(object sender, EventArgs e)
        {
            if (txtDesCat.Text!="")
            {
                if (txtNomCat.Text!="")
                {
                    ...
                    //ServiceProductos.ServiciosSoapClient micliente2 = new ServiceProductos.ServiciosSoapClient();
                    ServiceProductos.ProductosSoapClient clienteProductos = new ServiceProductos.ProductosSoapClient();

                    MessageBox.Show("Web Service Productos");
                    string nombre;
                    string desc;

                    nombre = txtNomCat.Text;
                    desc = txtDesCat.Text;

                    string cond = clienteProductos.InsertarCategoría(nombre, desc);

                    if (cond=="SI")
                    {
                        MostrarCat();
                        Limpiar();
                        MessageBox.Show("Categoria insertada");
                    }
                    if (cond == "NO")
                    {
                        MostrarCat();
                        Limpiar();
                        MessageBox.Show("Categoria no insertada, verificá tu servicio");
                    }
        ...
        private void btnSaveSub_Click(object sender, EventArgs e)
        {
            if (txtDesSub.Text != "")
            {
                if (txtNomSub.Text != "")
                {
                    ...
                    cat = Convert.ToInt32(cbxCat.SelectedValue);
                    nombre = txtNomSub.Text;
                    desc = txtDesSub.Text;

                    string cond = clienteProductos.InsertarSubcategoria(cat, nombre, desc);

                    if (cond == "SI")
                    {
                        MostrarSubCat();
                        LimpiarSub();
                        MessageBox.Show("Subcategoria insertada");
                    }
                    if (cond == "NO")
                    {
                        MostrarSubCat();
                        LimpiarSub();
                        MessageBox.Show("Subcategoria no insertada, verificá tu servicio");
                    }
                }
                else
                {
        ...
        private void MostrarEmpresa()
        {
            try
            {
                ...
                datalistadoEmpresa.Columns[2].Visible = false;
                ...
                datalistadoEmpresa.Columns[6].Visible = false;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
