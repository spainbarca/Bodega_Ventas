...
namespace Bodega_Ventas.MODULOS.INVENTARIO
{
    public partial class inventariokardexok : Form
    {
        ...
        {
            if (txtBuscarKardex.Text=="Buscar producto")
            {
                datalistadoKardex.Visible = false;
            }
            else
            {
        ...
        private void BuscarProductosKardex()
        {
            try
            {
                ...
                da = new SqlDataAdapter("buscar_productos_kardex", con);
                da.SelectCommand.CommandType = CommandType.StoredProcedure;
                da.SelectCommand.Parameters.AddWithValue("@letrab", txtBuscarKardex.Text);
                da.Fill(dt);
                datalistadoKardex.DataSource = dt;
                con.Close();

                datalistadoKardex.Columns[1].Visible = false;
                datalistadoKardex.Columns[3].Visible = false;
                datalistadoKardex.Columns[4].Visible = false;
                datalistadoKardex.Columns[6].Visible = false;
                datalistadoKardex.Columns[8].Visible = false;
                datalistadoKardex.Columns[10].Visible = false;
                datalistadoKardex.Columns[11].Visible = false;
                datalistadoKardex.Columns[12].Visible = false;
                datalistadoKardex.Columns[13].Visible = false;
                datalistadoKardex.Columns[14].Visible = false;
                datalistadoKardex.Columns[15].Visible = false;
                datalistadoKardex.Columns[16].Visible = false;
                datalistadoKardex.Columns[17].Visible = false;
                datalistadoKardex.Columns[18].Visible = false;
                datalistadoKardex.Columns[19].Visible = false;
                datalistadoKardex.Columns[20].Visible = false;
                datalistadoKardex.Columns[21].Visible = false;
                datalistadoKardex.Visible = true;
                CONEXION.Datatables_tamañoAuto.Multilinea(ref datalistadoKardex);
        ...
        private void datalistadoKardex_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            txtBuscarKardex.Text = datalistadoKardex.SelectedCells[2].Value.ToString();
            datalistadoKardex.Visible = false;
            MostrarReporteKardex();
        }


        REPORTES.REPORTES_KARDEX.Reporte_Kardex repKardex = new REPORTES.REPORTES_KARDEX.Reporte_Kardex();

        private void MostrarReporteKardex()
        {
            try
            {
                DataTable dt = new DataTable();
                ...
                da = new SqlDataAdapter("mostrar_kardex", con);
                da.SelectCommand.CommandType = CommandType.StoredProcedure;
                da.SelectCommand.Parameters.AddWithValue("@idProducto",datalistadoKardex.SelectedCells[1].Value.ToString());

                da.Fill(dt);
                con.Close();

                repKardex = new REPORTES.REPORTES_KARDEX.Reporte_Kardex();
                repKardex.DataSource = dt;
                repKardex.tablaKardex.DataSource = dt;
                reportViewerKardex.Report = repKardex;
